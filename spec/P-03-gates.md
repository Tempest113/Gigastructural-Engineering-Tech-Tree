...
- Gate detection is a **classification pass** over trigger blocks: the build inspects each
  technology's `potential` and `allow` conditions for recognised gate patterns using a
  checked-in, extensible gate-pattern registry. Unrecognised conditions MUST NOT be silently
  dropped — the build reports them.
