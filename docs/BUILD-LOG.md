...
## Session: client test infrastructure (Vitest, committed Playwright e2e, field-consumption invariant, polarity mutation harness)

**Context.** A survey found six schema fields (`DetailPayload.weight`, `.variants`, `.source`,
`.repositoryLink`, `.repeatableCostProgression`, `.overwriteDiff`) emitted, schema-validated, and
never read by any client code — two of them (`.weight`/D-4, `.repositoryLink`/D-5) backing locked
decisions this project had already recorded as done. Root cause: `client/` had zero tests of any
kind (no runner in `client/package.json`, no `.test.ts` files, every prior Playwright pass a
throwaway script run once and discarded) — the 1,541 pipeline tests are 100% Python and
structurally cannot see a client-consumption gap. This session built the harness, not the feature
fixes (explicitly fenced out — the `.weight` emitter etc. are a separate next task).

**Item 1 — Vitest runner + committed fixture dataset.** Added `vitest` to `client/`
(`npm run test`), a dedicated `client/vitest.config.ts` (node environment; kept separate from
`vite.config.ts` deliberately — see that file's own comment). Extracted three small pure-function
modules out of `main.ts`'s previously-inline, duplicated logic so they're independently testable:
`src/format.ts` (`formatCost`/`showCostPanel`/`formatRepeatableBadgeLabel`) and `src/search.ts`
(`tokenizeQuery`/`rankSearchMatches`); `main.ts` now imports and calls these instead of repeating
the same `Math.round(...).toLocaleString("en-US")` expression at three call sites. Found, but did
NOT fix (scope fence): the extracted `formatRepeatableBadgeLabel` faithfully preserves an existing
bug where an infinite repeatable's card badge renders bare `"∞"` instead of CLAUDE.md's own
`"×∞"` — flagged in the function's own docstring for a follow-up.

The fixture dataset (`client/tests/fixtures/dataset/`, regenerated by
`tools/build_client_fixture_dataset.py`) is a small, REAL, pipeline-computed subset of the actual
corpus — not hand-authored technology data, and not a synthetic mod source either (that path was
considered and rejected: a synthetic corpus would still need real icon files, since the build
hard-fails on a missing icon for a displayed technology, so it's no less vendor-dependent, just a
more fragile flavour of it). The script runs the real pipeline against `vendor/` (build_context +
build_base_dataset, same as `tools/build_dataset.py`), then mechanically slices the result down to
a curated technology per required shape (`tech_habitat_1` gated/negative-origin,
`giga_tech_blokkat_engineering_repeatable` repeatable+weight-gated, `giga_tech_maginot_world`
variants, the `tech_housing_2`/`tech_housing_agrarian_idyll` weight-modifier swap pair,
`tech_mega_engineering` alternative-group edges, one technology per `AvailabilityState`) plus every
edge directly touching one of them (single-hop neighbour closure, snapshotted against the ORIGINAL
curated set to avoid transitive cascade — an early version of the script tested membership against
the growing keep-set instead and ballooned to 209 technologies before this was caught). Slicing
relies on the schema's positional geometry contract (`technologies[i]` ↔ `nodePositions[i*2:i*2+2]`,
`edges[i]` ↔ `edgePolylines[i*12:(i+1)*12]`, confirmed against both `pipeline/geometry.py` and
`client/src/main.ts`'s own `FLOATS_PER_EDGE_POLYLINE`) — no geometry is recomputed, only reindexed.
Final fixture: 53 technologies, 57 edges, ~700KB total (diagnostics' own `unresolvableResearchPaths`
also trimmed to the kept set — not consumed by the client, but a full-corpus 490-entry list added
nothing to a "minimal" fixture).

Seed tests: `src/empireProfile.test.ts` (index/axis derivation against the real emitted-shape
axes, including the worked example from `dataset-types.ts`'s own doc comment), `src/format.test.ts`,
`src/search.test.ts`, `tests/dataset-fixture.test.ts` (proves the committed fixture itself is
shaped the way `dataset.ts`/`main.ts` actually expect, and that every curated shape it claims to
cover really is present — including a same-`refId` opposite-`negated` assertion on the housing
pair, straight from the real corpus). 40 tests, all passing. `tsc --noEmit` and `vite build` both
clean afterward.

**Item 2 — committed Playwright e2e check.** `client/tests/e2e/check.mjs` + its own README:
launches Chromium, loads a running dev/preview server, waits for `window.__tt` (main.ts's existing,
already-built test-hook API — nothing new added to main.ts for this), and asserts 0 console errors,
0 failed requests, and `.ok === true` from all six of `window.__tt`'s existing invariant checks
(`checkNameBounds`, `checkIndicatorBounds`, `checkGateLabelFontAndCollision`,
`checkEdgeEndpointsInCards`, `checkMinStubLength`, `checkTierBadgeMatchesBand` — matching the
task's name-bounds/indicator-bounds/gate-label-bounds/edge-containment/min-stub/tier-badge list
exactly). `playwright` is deliberately NOT a `client/package.json` devDependency — it needs the
real dataset (D-15-gated), so `npm ci` (what CI runs) must never try to install a browser download
for a check CI can never run; install once locally per the README. No CI workflow references it.

**Item 3 — field-consumption invariant.** `tools/check_field_consumption.py`: walks
`schema/generated/dataset-types.ts` (a brace-depth parse, not a real TS parser — sound for this
one generated, byte-for-byte-pinned file, per its own docstring) for the full 227-field list, greps
`client/src/*.ts` (excluding `.test.ts`) for each field name, and requires every unmatched field to
carry a reviewed entry in `config/consumed_field_annotations.txt`
(`displayed`/`key-only`/`not-needed`, `#`-justified, same posture as `config/icon_overrides.txt`).
An annotation for a field no longer in the schema is a hard error (confirmed: adding a bogus entry
then removing it round-trips correctly). Result: 133 consumed, 4 `displayed` (`Gate.kind`/`.refId`/
`.negated`/`.alternative` — the module docstring's own predicted false positives; `Gate.kind`/
`.alternative` were CONFIRMED to actually be naive-grep false positives, matching an unrelated
`edge.kind`/a local object literal's own `.alternative` property, not the schema field — annotating
them overrides that wrong verdict), 3 `key-only` (`BaseDataset.technologies.id`,
`DetailPayload.technologyId`, `SearchIndex.entries.technologyId` — used only as Map keys, never
displayed), 87 `not-needed`. That 87 is the six the original survey named PLUS more this check's
own first run turned up by itself: `BaseDataset.metadata` (P-10's "data as of" marker, never
rendered), `.categories` (P-4's derived filter list, no filter UI consumes it),
`.adjacency`/`.technologies[].labelPriority` (internal, superseded by baked-in geometry), and —
notably — schema-version validation is unimplemented on ALL FIVE artefacts (`common.schema.json`'s
own doc comment: the client "MUST refuse to render an unsupported version," and nothing in
`client/src/` checks any `schemaVersion` field at all). Wired into
`.github/workflows/client-tests.yml` alongside Vitest — both vendor-independent, both real CI jobs.

**Item 4 — polarity mutation harness.** `tests/test_gate_patterns_polarity_mutations.py`, following
`tests/clausewitz/test_roundtrip_detects_mutations.py`'s established pattern: monkeypatches
`classify_weight_gate_condition` to drop `invert_polarity=True`, and asserts the housing-pair
assertion THEN FAILS (confirmed both directions inside the test itself: passes unpatched, fails
patched, passes again once unpatched — ruling out test-order pollution as the cause of either
result). Found and fixed a real trap while writing this: an `unwrapped.negated != wrapped.negated`
assertion is satisfied by BOTH the correct classifier and its exact polarity-inverted opposite (an
inverted classifier still produces two mutually-opposite values, just both flipped from reality) —
caught by first watching the harness fail to catch its own mutation, then pinning the ABSOLUTE
expected value for each side instead of just their mutual inequality; `test_gate_patterns.py`'s own
`KNOWN_OPPOSITE_POLARITY_WEIGHT_CONDITION_PAIRS` table was corrected the same way. Also added:
`test_negating_the_source_condition_flips_negated_at_every_wrapper_depth`, a 17-case parametrised
table over depth-1/2/3 AND/OR/NOT/NOR wrapper combinations, generalising the one hand-picked case
`test_double_negation_of_literal_no_value_produces_a_real_gate` covered.

**Untouched, confirmed**: node set, per-state population, D-10's three figures (31/973, 16/973,
53) — this task changes no pipeline output, only test/CI infrastructure. `docs/DEFECTS.md` gained
two new sections ("Unread fields", "Polarity mutation harness") recording both defect classes for
future recognition.
