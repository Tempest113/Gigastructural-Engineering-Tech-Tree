...
/** P-13: availability MUST be an enumerated state, never a boolean. 'uncertain' means the partial trigger evaluator could not resolve the condition for this profile (D-10's 'unknown', propagated) -- it is not a variant of 'locked' and must never be presented as though it were either available or locked. 'config-gated' (added by D-10's giga_tech_repeatable_*_cap application, spec/decisions.md) means the technology's potential resolved DEFINITIVELY to false because of a mod-configuration toggle (a has_global_flag matching pipeline.trigger_text.MOD_CONFIG_TOGGLE_SUFFIXES), not because of anything about the empire being played -- distinct from 'locked', which elsewhere always means an empire-state property is what's blocking the technology. 'weight-gated' (D-10's Extension, a later session correcting Item 2b's overbroad weight-modifier folding) means a zero-factor `weight_modifier` condition currently fires or can't be resolved: 'Not currently offered in the research draw. This depends on your empire's circumstances rather than its type -- the condition is shown below.' It is deliberately NOT 'locked' -- a static evaluator can't see `give_technology`, events, special projects, archaeology or relics, any of which can grant the technology outside the weighted draw regardless of weight -- and does NOT fold into D-10 uncertainty accounting, exactly like 'config-gated'; the research-path builder (P-12.9) treats it as VIABLE, unlike 'locked'/'config-gated'. This $defs key was named 'ThreeState' before 'config-gated' was added; renamed then to a value-count-independent name deliberately, so this fifth value ('weight-gated') didn't need another rename to arrive. */
export type AvailabilityState = "available" | "locked" | "uncertain" | "config-gated" | "weight-gated";
...
export interface Gate {
  ...
  icon: null | IconRef;
  /** Localised gate label, e.g. 'Needs Cosmogenesis' or 'Unavailable to Wilderness Origin'. Sourced from the mod's own localisation, never hard-coded (P-3). */
  label: string;
  /** Negative gates (a later session): true iff the underlying condition's true polarity (pipeline.gate_patterns._leaf_negated's 3-channel XOR over NOT/NOR wrapping, the != operator, and a literal '= no' value) is 'must NOT have/be X' rather than 'must have/be X' -- e.g. `is_wilderness_empire = no` (31 real technologies, tech_habitat_1/2/3 among them: a wilderness player can never research these). `label` already carries this in its wording ('Unavailable to X' vs 'Needs X'/'or: X'); this field exposes the same fact structurally for a client that wants to style or filter on it rather than parse the label text. Does not affect gate priority (order_gates sorts on kind alone, polarity-independent -- D-3). */
  negated: boolean;
...
export interface EmpireOverlay {
  ...
  availability: { [key: string]: {
    ...
    /** Required (non-null) when state is 'locked', 'uncertain', 'config-gated' or 'weight-gated'; null when 'available'. Two valid origins for a locked reason (P-13): trigger-derived (the specific failed condition) or structure-derived (no edge of any kind reaches this technology for this profile -- P-16). Both are ordinary strings in this one field; a structure-derived reason MUST be phrased in terms of reachability, never as though a trigger failed. An uncertain reason always carries the unresolved trigger's source text. A config-gated reason carries the mod-configuration flag's trigger text (e.g. 'has_global_flag = giga_tech_repeatable_dyson_swarm_capped_r') -- phrased around the game OPTION responsible, never as though the empire itself were the obstacle. A weight-gated reason carries the zero-factor `weight_modifier` condition's phrasing (pipeline.trigger_text.describe_condition, raw-text fallback) -- for the `always = yes` case, phrased around the technology being obtained outside the normal research draw; otherwise phrased around the draw itself, never speculating about which unmodelled mechanism (event/give_technology/special project/archaeology/relic) might actually grant it. */
    reason: null | string;
  ...
  researchPaths: { [key: string]: {
    ...
    steps?: ({
      ...
      /** Never locked/config-gated -- a step whose own state resolves to either is excluded upstream (a plain locked prerequisite makes the whole path unavailable; a config-gated technology can only ever be the path's own target, per the sink property). 'weight-gated' IS a viable step (D-10's Extension): unlike locked/config-gated, the technology remains eventually researchable, so it's treated the same way 'uncertain' is here -- a real step, not a route-breaker. */
      availabilityState: "available" | "uncertain" | "weight-gated";
    ...
    /** Empty when totalCostIsEstimate is false. Can carry more than one reason at once. 'weight-gated-step' is set whenever any step (or the target itself) is 'weight-gated' -- distinct from 'uncertain-availability', since a weight-gated step is a determinate, known-not-currently-drawn fact, not an undecidable one, but the total is still an estimate because the weight gate could lift. */
    estimateReasons?: ("uncertain-availability" | "unresolved-cost" | "weight-gated-step")[];
